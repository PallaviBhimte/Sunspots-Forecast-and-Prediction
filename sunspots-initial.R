...
data_sunspots <- read_csv("sunspots.csv",col_names = TRUE)

# str(data_sunspots$Date)

# class(data_sunspots)

data_sunspots <- tail(data_sunspots,841)
data_sunspots
...
sunspotsts <- ts(data_sunspots$`Monthly Mean Total Sunspot Number` ,start=c(1951, 1), end=c(2021, 1), frequency = 12)
...
model.113_OF = arima(sunspotsts,order=c(1,1,3),method='ML')
coeftest(model.113_OF)
